...
class BlockTable:
    """One page per block, shared by every head: ``[request, slot]``.

    ``RaggedBlockTable`` is the variant that gives each head group its own
    pages; ``ragged`` says which one a table is without importing either.
    """
    ...
    def _allocate_buffers(self) -> None:
        """Allocate the block table, its per-row fill counts, and the slot
        mapping. Overridden to give each buffer a head-group axis."""
        self.block_table = self._make_buffer(
            self.max_num_reqs, self.max_num_blocks_per_req, dtype=torch.int32
        )
        self.num_blocks_per_row = np.zeros(self.max_num_reqs, dtype=np.int32)
...
class MultiGroupBlockTable:
    ...
    def __init__(
        ...
        # Ragged paging collapses the list to one table: ``num_head_groups`` is
        # the total (per-layer x layers), so the layer dimension is already
        # absorbed into the flat group index. Which class the group axis lives
        # in is decided once, here.
